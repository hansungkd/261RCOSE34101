...
/* Run one SJF variant and save its metric result. */
static int run_sjf(int preemptive, GanttChart *gantt_chart, ScheduleResult *result)
{
...
/* Build only the Non-Preemptive SJF metric result for the comparison table. */
int scheduler_nonpreemptive_sjf_result(ScheduleResult *result)
{
    return run_sjf(0, 0, result);
}

/* Build only the Preemptive SJF metric result for the comparison table. */
int scheduler_preemptive_sjf_result(ScheduleResult *result)
{
    return run_sjf(1, 0, result);
}
...
void scheduler_run_nonpreemptive_sjf(void)
{
    ...
    ran = run_sjf(0, &gantt_chart, &result);
    if (ran == 0) {
        return;
    }
...
void scheduler_run_preemptive_sjf(void)
{
    ...
    ran = run_sjf(1, &gantt_chart, &result);
    if (ran == 0) {
        return;
    }
